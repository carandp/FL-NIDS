...
class NIDSTrainer(Executor):
    ...
    def _make_zero_update_reply(self, reference_weights: dict) -> Shareable:
        zero_diff = {k: torch.zeros_like(v).cpu().numpy() for k, v in reference_weights.items()}
        out_dxo = DXO(
    ...
    def _apply_dp_to_weight_diff(self, weight_diff: dict, fl_ctx: FLContext, current_round: int) -> dict:
        if not self.dp_enabled:
            return weight_diff

        if self.dp_clip_norm <= 0:
            raise ValueError("dp_clip_norm must be > 0 when dp_enabled=True")

        # Convert to tensors to clip/noise in a numerically stable way.
        tensors = {k: torch.from_numpy(v).float() for k, v in weight_diff.items()}

        sq_norm = 0.0
        for t in tensors.values():
            sq_norm += float(torch.sum(t * t).item())
        global_norm = math.sqrt(sq_norm)
        clip_coef = min(1.0, self.dp_clip_norm / (global_norm + 1e-12))

        noise_std = self.dp_noise_multiplier * self.dp_clip_norm
        for k, t in tensors.items():
            t = t * clip_coef
            if self.dp_noise_multiplier > 0:
                noise = torch.randn(t.shape, generator=self.dp_generator, dtype=t.dtype)
                t = t + noise * noise_std
            tensors[k] = t

        self.log_info(
            fl_ctx,
            f"Round {current_round} — DP applied: clip_norm={self.dp_clip_norm:.4f}, "
            f"noise_multiplier={self.dp_noise_multiplier:.4f}, global_update_norm={global_norm:.4f}, "
    ...
    def _train(
        ...
        for epoch in range(self.local_epochs):
            ...
            if not math.isfinite(epoch_loss):
                self.log_error(fl_ctx, f"Non-finite epoch loss detected at round {current_round}: {epoch_loss}")
                return self._make_zero_update_reply(global_weights)
        ...
        for k, v in local_weights.items():
            if not torch.isfinite(v).all().item():
                self.log_error(fl_ctx, f"Non-finite model parameter detected at round {current_round}: {k}")
                return self._make_zero_update_reply(global_weights)
        weight_diff = {
            k: (local_weights[k].cpu() - global_weights[k]).numpy()
            for k in local_weights
        }
        weight_diff = self._apply_dp_to_weight_diff(weight_diff, fl_ctx, current_round)
        for k, v in weight_diff.items():
            if not torch.isfinite(torch.from_numpy(v)).all().item():
                self.log_error(fl_ctx, f"Non-finite weight diff detected at round {current_round}: {k}")
                return self._make_zero_update_reply(global_weights)
