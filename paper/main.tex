...
\autoref{tab:low_noise_epsilon} and \autoref{fig:privacy_utility_tradeoff} summarize the impact of varying Differential Privacy noise multiplier configurations on federated intrusion detection performance and accumulated client privacy budgets.
...
Conversely, smaller noise multiplier values partially recover classification capability, particularly for the Macro F1-score. Nevertheless, these configurations result in extremely large accumulated privacy budgets, reaching impractically high $\epsilon$ values despite improved utility metrics.
...
Client 0 exhibited the highest degree of instability, presenting the largest fluctuations in both Macro F1-score and PR-AUC across communication rounds. This behavior may be associated with the severe class imbalance present within the corresponding local dataset partition, where the combination of limited malicious traffic representation and repeated noisy perturbation amplified optimization sensitivity.
...
The experimental results demonstrate that the Differential Privacy noise multiplier strongly influences both intrusion detection performance and accumulated privacy budget growth.
...
Conversely, smaller noise multiplier values partially improved model utility by reducing the magnitude of the injected perturbation. Despite these improvements, the corresponding privacy budgets increased to impractically large $\epsilon$ values, indicating weak effective privacy guarantees.

The observed relationship between noise multiplier and utility was also non-linear across experiments, suggesting that Differential Privacy perturbation interacts sensitively with the heterogeneous and imbalanced federated training environment used in this work.
...
The results further suggest that the combination of graph neural network embeddings, Transformer-based reconstruction, severe class imbalance, and heterogeneous federated client distributions significantly increases optimization sensitivity under noisy training conditions.
...
SecAgg maintained relatively strong intrusion detection capability while significantly increasing runtime and data transmission requirements due to HE operations and ciphertext expansion. In contrast, the evaluated DP configurations introduced substantially greater optimization instability and weaker utility preservation, while still accumulating impractically large privacy budgets under several experimental settings.
...
The experimental results indicate that heterogeneous client data distributions significantly influenced federated optimization behavior, embedding representation quality, and privacy budget accumulation throughout training.

Clients containing smaller or severely imbalanced datasets exhibited greater instability in both performance metrics and embedding space separability, particularly under privacy-preserving training configurations. These effects became more pronounced during DP experiments, where noisy perturbation amplified optimization sensitivity across clients with limited attack representation.

The observed variability highlights one of the primary challenges of real-world federated intrusion detection environments, where participating organizations may contain substantially different traffic distributions, dataset sizes, and attack frequencies. Under these conditions, maintaining balanced model performance and privacy guarantees across all federated participants becomes considerably more difficult.

\subsection{Practical Implications for Real Deployments}

The obtained results suggest that FL can provide a practical foundation for collaborative intrusion detection across distributed organizations without requiring centralized sharing of raw network traffic data. The evaluated GraphIDS architecture maintained relatively strong detection capability under federated training while preserving meaningful latent traffic representations across clients.

However, the experiments also demonstrate that privacy-preserving mechanisms introduce substantial operational trade-offs. SecAgg significantly increased communication and computational cost, while the evaluated DP strategy produced unstable optimization behavior and limited practical privacy guarantees.

These findings indicate that real-world deployment of privacy-preserving federated intrusion detection systems requires careful consideration of scalability constraints, client heterogeneity, and the balance between confidentiality protection and detection effectiveness.

\subsection{Limitations of the Study}

This study presents several limitations that should be considered when interpreting the experimental results. The federated environment was evaluated using a limited number of simulated clients under a controlled experimental configuration, which may not fully represent the complexity of large-scale real-world deployments.

Additionally, the experiments focused exclusively on the FedAvg aggregation strategy together with a fixed local epoch training configuration. Alternative aggregation methods, adaptive optimization strategies, or personalized federated learning approaches may produce different stability and performance behavior under heterogeneous client conditions.

The DP evaluation was also limited to client-side Gaussian perturbation using fixed noise multiplier configurations. More advanced privacy-preserving approaches, such as adaptive clipping, personalized privacy accounting, or secure aggregation-assisted DP, were not explored within the scope of this work.

\subsection{Future Work}

Future work may explore alternative federated optimization strategies capable of improving stability under heterogeneous client distributions, including personalized federated learning, adaptive client weighting, or asynchronous aggregation approaches.

Additional research is also needed to evaluate more advanced DP mechanisms for graph-based intrusion detection systems. Techniques such as adaptive gradient clipping, privacy amplification strategies, or secure aggregation-assisted DP may help improve the observed privacy-utility trade-off.

Finally, future experiments should evaluate larger-scale federated environments with additional clients, more diverse traffic distributions, and real-world deployment constraints in order to further analyze the scalability and operational feasibility of privacy-preserving federated intrusion detection systems.


\section{Conclusion}

This work presented and evaluated a privacy-preserving federated learning architecture for graph-based intrusion detection using the GraphIDS framework integrated with Federated Averaging, homomorphic encryption-based Secure Aggregation, and Differential Privacy mechanisms within the NVIDIA FLARE environment.

The experimental results demonstrated that federated learning is a viable approach for distributed graph-based intrusion detection under heterogeneous non-IID client conditions. The federated GraphIDS configuration preserved performance relatively close to the centralized baseline while maintaining stable collaborative optimization behavior and meaningful latent traffic representations without requiring centralized access to raw network traffic data.

The integration of homomorphic encryption-based Secure Aggregation successfully protected client model updates during federated training by preventing the aggregation server from accessing plaintext parameters. Although the encrypted pipeline introduced substantial computational and communication overhead due to ciphertext expansion and homomorphic encryption operations, the resulting model preserved effective intrusion detection capability together with stable convergence behavior.

In contrast, the evaluated client-side Differential Privacy configurations produced severe optimization instability, oscillatory metric behavior, and extremely large accumulated privacy budgets across multiple experimental settings. The results suggest that direct noisy perturbation strategies may be insufficient for complex graph-based intrusion detection architectures operating under highly heterogeneous and imbalanced federated environments.
...
Future work should explore adaptive privacy-preserving optimization strategies, alternative federated aggregation methods, and larger-scale heterogeneous environments to further improve the practicality and scalability of secure federated intrusion detection systems.
